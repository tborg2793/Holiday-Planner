...
<html >
  ...
  <body>
    ...
    <div class="wrapper">
	<div class="container">
		<h1>Welcome - Holiday Planner Social Network</h1>
		
		<form class="form" method="post" action="" name="form1">
			<input type="text" placeholder="Username" id="myusername" name="myusername" class="myusername" value="test1@test.com">
			...
			<label onmouseover="" style="cursor: pointer;" onclick="window.location.href = 'register.php'">Don't have an Account? Click here to Register!</label>
		</form>
	</div>
	...
	$('#login-button').click(function(){ // Create `click` event function for login
		...
		if(username.val() != '' && password.val() != ''){ // Check the username and password values is not empty and make the ajax request
			...
			alert(responseText);
				...
				{
					alert('Account not yet active. Please check email to activate your account');
				}
